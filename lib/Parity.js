...
class Parity extends JsonRpc {
     ...
     * @returns {Promise<*>}
     */
     async deploy ({from, to = null, data, gas = null, password}) {

         const method = 'personal_sendTransaction';

         const tx = {from, to, data};
         if (gas !== null) tx.gas = gas;


         const body = JsonRPC.create(method, [tx, password]);
         //console.debug(body);
         try {
